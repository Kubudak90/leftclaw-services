...
export default function PfpPage() {
  ...
  const [step, setStep] = useState<"idle" | "signing" | "burning" | "spending" | "generating" | "done" | "error">("idle");
  ...
  const handleCvGenerate = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        if (res.status === 402) throw new Error(`Not enough ClawdViction. You have ${(data.currentBalance || 0).toLocaleString()} CV, need ${PFP_CV_COST.toLocaleString()}.`);
  ...
  const handleGenerate = () => { paymentMethod === "cv" ? handleCvGenerate() : handleBurnGenerate(); };
  ...
  const handleReset = () => { setStep("idle"); setError(null); setGeneratedImage(null); setBurnInfo(null); setCvInfo(null); setPrompt(""); };
        ...
        {step !== "done" && (
          <>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <button className={`btn btn-sm flex-1 ${paymentMethod === "cv" ? "btn-primary" : "btn-outline"}`} onClick={() => setPaymentMethod("cv")} disabled={busy}>⚡ ClawdViction</button>
                <button className={`btn btn-sm flex-1 ${paymentMethod === "burn" ? "btn-primary" : "btn-outline"}`} onClick={() => setPaymentMethod("burn")} disabled={busy}>🔥 Burn CLAWD</button>
              </div>
            </div>
            ...
            {paymentMethod === "cv" && insufficientCv && (
              <div className="alert alert-error mb-4">
                <span>Not enough ClawdViction. You have {(cvBalance || 0).toLocaleString()} CV, need {PFP_CV_COST.toLocaleString()}. <a href="https://clawdviction.vercel.app/stake" target="_blank" rel="noopener" className="underline">Stake CLAWD →</a></span>
              </div>
            )}
            ...
            >
              ...
              {step === "signing" ? "Sign message in wallet..." :
               ...
               paymentMethod === "cv" ? `⚡ Spend ${PFP_CV_COST.toLocaleString()} CV & Generate` :
            ...
            <p className="text-center text-xs opacity-40 mt-6">
              {paymentMethod === "burn" ? "CLAWD is burned (sent to 0xdead) — deflationary and non-refundable." : "ClawdViction is earned by staking CLAWD. No tokens are burned."}

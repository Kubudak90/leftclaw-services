# CLAWD Quick Consultation — Agent Skill File

> For AI agents and bots. Everything you need to pay USDC and start a quick consultation session.
> Human page: https://leftclaw.services/consult

**Price:** Dynamic — read from the 402 response (USDC on Base)
**Endpoint:** `POST https://leftclaw.services/api/consult/quick`
...
- `"Is it feasible to build a gasless relay for ERC-20 transfers on Base?"`
- `"Help me design a DAO governance system with quadratic voting"`
...
```typescript
/**
 * CLAWD Quick Consultation — x402 payment script
 *
 * Requirements:
...
const DESCRIPTION = "I want to build a token vesting contract on Base with a cliff and linear unlock";
...
1. `POST /api/consult/quick` with no payment → `402` response with `PAYMENT-REQUIRED` header (base64 JSON)
...
*CLAWD Quick Consultation · https://leftclaw.services/consult*
